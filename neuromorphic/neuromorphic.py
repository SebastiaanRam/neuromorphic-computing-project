...
previous_reward = 0
a_inputs = [previous_a_input]
r_inputs = [previous_reward]
...
for episode in range(num_episodes):
    ...
    for step in range(max_episode_steps):
        ...
        # Use recent mean
        previous_a_input = np.mean(a_inputs[-1:], axis=0)
        previous_reward = np.mean(r_inputs[-1:], axis=0)
        # Compute activities of motor cortex population
        ...
        a_inputs.append(current_a_input)
        r_inputs.append(current_reward)
        if done:
            break
